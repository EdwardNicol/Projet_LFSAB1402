...
<<<<<<< HEAD
	 Audio
=======
        case {Flatten Music} of nil %On fait en sorte de n'avoir qu'une seule liste et pas des listes de listes pour faciliter l'écriture du programme
        then nil
        ...
        case H
        of voix then %Deux cas sont distingués si on a une voix, si c'est un silence ou un échantillon
       ...
       [] transformation then %Si c'est une transformation, nous appliquons la fonction Interprete qui renvoie une voix qui peut être utilisé par Mix
       ...
       [] fondu(ouverture:S1 fermeture:S2 Music) then %Retourne un vecteur audio avec une partie "adoucie" entre S1 et S2
	     ...
	     fun{Fondu L AccOuv Dur1 AccFer Dur2 Return}
	        ...
	        elseif AccOuv>0.0 then {Fondu L.2 AccOuv-1 Dur1 AccFer Dur2 ((Dur1-AccOuv)/Dur1)|Return %Addouci le début de la musique
		elseif AccOuv =<0.0 and then {Length L}>AccFer then {Fondu L.2 AccOuv Dur1 AccFer Dur2 Return}%Rien n'est effectué, on avance dans le vecteur
	    ...
	    fun{CountNotes Partition}% Compte le nombre de notes dans une partition
		  ...
		  fun {SubCountNotes Partition Acc}
			...
			[] duree(secondes:F P) then {SubCountNotes T Acc+{SubCountNotes P 0.0}}
		     ...
		     [] Mono then % Cas ou la partition ne comporte qu'un seul Ã©lÃ©ment
			case Mono
...
<<<<<<< HEAD
      % seul qui ateste de la validitÃ© de votre implÃ©mentation.
